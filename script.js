...
/**
 * Find the upcoming (nearest) deadline and update the UI.
 * @async
 * @returns {Promise<void>}
 */
async function updateUpcomingDeadline() {
  try {
    ...
    renderDeadlineResult(deadlines);
  } catch (error) {
...
/**
 * Fetch all deadlines across task categories that are today or later.
 * @async
 * @returns {Promise<Date[]>} array of valid Date objects
 */
...
/**
 * Render the nearest deadline or fallback text into #currentDate.
 * @param {Date[]} deadlines
 * @returns {void}
 */
function renderDeadlineResult(deadlines) {
  const el = document.getElementById("currentDate");
  if (deadlines.length > 0) {
    const minDate = new Date(Math.min(...deadlines));
    el.textContent = minDate.toLocaleDateString("en-GB", {
      year: "numeric", month: "long", day: "numeric"
    });
  } else {
    el.textContent = "No upcoming deadline";
  }
}
