/**
 * Return a placeholder template when a category has no tasks.
 * @param {string} category - The category name (e.g., "To do", "In progress").
 * @returns {string} HTML string that shows an empty-state message for the category.
 */
...
/**
 * Render a single task card template for the board.
 * - Computes subtasks progress and progress bar width.
 * - Renders up to a maximum of three assigned users, then "+X" for overflow.
 */

/**
 * Ensure userData is always an array.
 * @param {Array<{id:string, initials:string, color:string}>} userData
 * @returns {Array<{id:string, initials:string, color:string}>}
 */
...
/**
 * Calculate subtasks progress counts.
 * @param {Object} task
 * @returns {{completed:number,total:number}}
 */
function computeSubtaskStats(task) {
...
/**
 * Calculate progress bar width (SVG width default 180).
 * @param {number} completed
 * @param {number} total
 * @param {number} [fullWidth=180]
 * @returns {number}
 */
function calcProgressWidth(completed, total, fullWidth = 180) {
  return total > 0 ? (completed / total) * fullWidth : 0;
}

/**
 * Build Assigned-To avatars HTML (maxVisible + "+X").
 * @param {string[]} assignedIds
 * @param {Array<{id:string, initials:string, color:string}>} userData
 * @param {number} [maxVisible=3]
 * @returns {string}
 */
function buildAssignedHTML(assignedIds, userData, maxVisible = 3) {
  const assigned = assignedIds || [];
  const visibleUsers = assigned.slice(0, maxVisible);
  const hiddenCount = assigned.length - maxVisible;

  let html = visibleUsers.map(id => {
  ...
  if (hiddenCount > 0) {
    html += `<div class="taskSingleContact">
...
/**
 * Return an inline SVG corresponding to the task priority.
...
function taskTemplate(task, userData) {
  ...
  const { completed, total } = computeSubtaskStats(task);
  const progressBarWidth = calcProgressWidth(completed, total, 180);

  const assignedHTML = buildAssignedHTML(task.assigned_to || [], users, 3);
...
/**
 * Technical Task overlay template.
 * @returns {string} HTML string of the "Technical Task" overlay.
 */
...
/**
 * User Story overlay template.
 * @returns {string} HTML string of the "User Story" overlay.
 */
