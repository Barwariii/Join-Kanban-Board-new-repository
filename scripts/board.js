// --- board.js (modified, drag/drop removed) ---
...
async function openTaskBoard() {
  ...
  await loadScript('../scripts/board_overlay_add_task.js');
    ...
    // 4) Show the overlay (only now)
  // overlay.style.display = 'block';
  
  // ➕ HIER: Medium als Default setzen (nur wenn noch nichts aktiv)
  if (typeof window.setDefaultPriorityMedium === 'function') {
    window.setDefaultPriorityMedium();
  }

  // 4) Show overlay and init
  overlay.style.display = 'block';
...
// =====================================
// Edit Task Overlay (Open/Get/Save/Close)
// =====================================
...
// =====================================
// Priority Selection (Edit Overlay)
// =====================================
 ...
 * @param {number} index - Index of the subtask in editTaskSubtasks array.
 */
function toggleSubtaskCompletion(index) {
  if (editTaskSubtasks[index]) {
    ...
    renderSubtasksForEdit(); // سيتم استدعاء renderSubtasks بداخلها
  }
}
...
// =====================================
// Save Edited Task (full PUT version)
// =====================================
async function saveEditedTask() {
  if (!currentEditedTaskId) return;
